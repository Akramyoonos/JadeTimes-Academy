...
const navLinks = [
  ...
  { href: '/blog',                text: 'Blog' },                 // match your route casing
  { href: '/contact_us',          text: 'Contact Us' },
  { href: '/alumni',              text: 'Alumni' },
  { href: '/StudentResources',    text: 'Student Resources' },    // keep this exact casing if your route is /StudentResources
...
const LanguageSelector = ({ isOpen, onToggle }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        {languages.map((lang) => (
          <a key={lang} href="/" className="block px-4 py-2 text-sm text-gray-200 hover:bg-gray-700 transition-colors duration-200">
            {lang}
          </a>
        ))}
      </div>
...
const SearchModal = ({ isOpen, onClose }) => {
  ...
  const handleQuickSearch = (term) => setSearchTerm(term);

  const filteredQuickSearches = quickSearches.filter(term =>
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex justify-center items-start pt-16 sm:pt-24 transition-opacity duration-300 ${
    ...
    >
      ...
      <div
        ref={modalRef}
        className={`relative w-full max-w-2xl bg-gray-800 rounded-lg shadow-2xl transition-transform duration-300 ${
      ...
      >
        <div className="p-6">
          <div className="relative">
            <input
              type="text"
              placeholder="e.g., 'Filmmaking Workshops'"
              className="w-full bg-gray-700 text-white placeholder-gray-400 pl-5 pr-16 py-4 rounded-md focus:outline-none focus:ring-2 focus:ring-yellow-400 transition-all duration-300 text-lg"
              value={searchTerm}
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 bg-yellow-400 hover:bg-yellow-500 text-gray-900 font-bold py-2 px-4 rounded-md transition-colors duration-300"
            >
        ...
        {searchTerm === '' ? (
              ...
              <div className="flex flex-wrap gap-3">
                {quickSearches.map(term => (
                  <button
                    ...
                    className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-gray-200 rounded-full text-sm transition-colors duration-200"
                  >
                    {term}
                  </button>
                ))}
                ...
                <ul className="space-y-3">
                  {recentSearches.map(term => (
                    <li key={term}>
                      <button
                        onClick={() => handleQuickSearch(term)}
                        className="flex items-center gap-3 text-gray-300 hover:text-yellow-400 transition-colors duration-200 w-full text-left"
                      >
          ...
          <div className="border-t border-gray-700 p-6">
            <h4 className="text-sm font-semibold text-gray-400 mb-4">Search Results</h4>
            {filteredQuickSearches.length > 0 || filteredRecentSearches.length > 0 ? (
              <>
                {filteredQuickSearches.length > 0 && (
                  <div className="mb-4">
                    <h5 className="text-xs font-semibold text-gray-500 mb-2">QUICK SEARCHES</h5>
                    <div className="flex flex-wrap gap-3">
                      {filteredQuickSearches.map(term => (
                        <button
                          key={term}
                          onClick={() => handleQuickSearch(term)}
                          className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-gray-200 rounded-full text-sm transition-colors duration-200"
                ...
                {filteredRecentSearches.length > 0 && (
                  <div>
                    <h5 className="text-xs font-semibold text-gray-500 mb-2">RECENT SEARCHES</h5>
                    <ul className="space-y-3">
