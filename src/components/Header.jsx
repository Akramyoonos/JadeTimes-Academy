...
const SearchModal = ({ isOpen, onClose }) => {
    ...
    const handleSearch = () => {
        if (searchTerm.trim() !== '') {
            // For now, let's just log it. A real implementation would navigate.
            ...
            <div ref={modalRef} className={`relative w-full max-w-2xl bg-gray-800 rounded-lg shadow-2xl transition-transform duration-300 ${isOpen ? 'scale-100' : 'scale-95'}`}>
                <div className="p-6">
                    <div className="relative">
                        ...
                        />
                        <button onClick={handleSearch} className="absolute right-2 top-1/2 -translate-y-1/2 bg-yellow-400 hover:bg-yellow-500 text-gray-900 font-bold py-2 px-4 rounded-md transition-colors duration-300">
                        ...
                        <div className="border-t border-gray-700 p-6">
                            ...
                            <div className="flex flex-wrap gap-3">
                                {quickSearches.map(term => (
                                    <button key={term} onClick={() => handleQuickSearch(term)} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-gray-200 rounded-full text-sm transition-colors duration-200">{term}</button>
                                ))}
                            </div>
                        </div>
                            ...
                            <div className="border-t border-gray-700 p-6">
                                ...
                                <ul className="space-y-3">
                                    {recentSearches.map(term => (
                                        <li key={term}>
                                            <button onClick={() => handleQuickSearch(term)} className="flex items-center gap-3 text-gray-300 hover:text-yellow-400 transition-colors duration-200 w-full text-left">
        ...
        <div className={`fixed inset-0 z-40 transition-opacity duration-300 lg:hidden ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            ...
            <div className={`absolute top-0 right-0 h-full w-80 bg-gray-900 shadow-2xl p-6 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <ul className="flex flex-col gap-5 mt-12">
                    {navLinks.map((link) => (
                        <li key={link.text}>
                            <a className="block py-2 text-xl font-medium text-gray-300 hover:text-yellow-400 transition-colors duration-300" href={link.href}>{link.text}</a>
                        </li>
                    ))}
                </ul>
